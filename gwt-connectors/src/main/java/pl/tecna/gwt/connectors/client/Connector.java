...
public class Connector {
	...
	/**
	 * Connector is a rectilinear connection that connects two {@link EndPoint).
	 * Connector is made of a couple of {@link Section} and it always starts 
	 * and ends on {@link EndPoint}. 
	 *
	...
	private void init (int startLeft, int startTop, int endLeft, int endTop, 
			SectionDecoration startDecoration, SectionDecoration endDecoration, ArrayList<CornerPoint> cornerPoints) {
	...
	/**
	 * Calculates positions of Connector's sections and shows Connector on 
	 * a given panel. The Connector is represented by horizontal or vertical
	 * lines which are {@link Section}s. The panel argument's type must be 
	 * AbsolutePanel.
	 * <p>
	 * This method also add all necessary {@link Point}s: {@link EndPoint}s 
	 * at the beginning and at the end of the Connector and {@link CornerPoint}s
	 * at the Connector's corners. 
	 * <p>
	 * The way sections are generated: </br>
	 * Let "width" � a width of rectangle drown on connectors start point 
	 * and end point </br>
	 * Let "height" � a height of rectangle drown on connectors start point
	 * and end point </br>
	 * If ("width" < "height") � the connection contains two vertical
	 *  sections and one horizontal section. </br>
	 * If ("height" < "width") � the connection contains two horizontal
	 * sections and one vertical section.
	 * 
	...
	/**
	 * Calculates values of local variables cornerPoint1Left, cornerPoint1Top, cornerPoint2Left, cornerPoint2Top 
	 * to draw a standard rectilinear tree-sections connector. Points positions depends on connector's
	 * width and height. If the width is less than the height the connection contains two vertical 
	 * sections and one horizontal section. If the width is greater than height the connection contains
	...
	public Section findNeighborSection(
			Section section, Point point) {
		Section sec = null;
		for (int i = 0; i < sections.size(); i++) {
...
//		Log.fine("getNextSection");
	...
	/**
	 * Creates new {@link Section} when {@link Connector} is connected to the {@link ConnectionPoint} </br>
	...
	/**
	 * Retrieve list of {@link CornerPoints} from list of {@link Section} (without {@link Connector} end points)
	 * @param sectionList 
	 * @return
	 */
	...
	/**
	 * Retrieve list of {@link CornerPoints} from list of {@link Section} of this {@link Connector} (without {@link Connector}'s end points)
	 * @return
	 */
	...
	/**
	 * Merges two {@link Section} on end of {@link Connector} </br>
	 * {@link Connector} sections length must be greater equals 3, and last 
	 * section length must be lesser than defined length (default - 10)
	 * 
	...
	/**
	 * Merges two {@link Section} on start of {@link Connector} </br>
	 * {@link Connector} sections length must be greater equals 3, and first 
	 * section length must be lesser than defined length (default - 10)
	 * 
	 * @param endSection last {@link Section} of {@link Connection}
	 * @return
	...
	/**
	 * Check if some sections in connection are mergeable (some section's length is shorter than defined value) and merges them.
	...
	void evadeShape(Shape shape, List<Section> overlapSections, List<CornerPoint> corners) {
	  ...
	  if (shape.isEnableOverlap()) {
		...
		/*
		 * define from which direction shape is leaved by overlap sections
		 * 0 - left
		 * 1 - top 
		 * 2 - right
		 * 3 - bottom
	...
	public void rememberSectionsPositions() {
		...
		for (Section section : sections) {
			savedSectionsData.add(new SectionData(
					...
					section.startPoint.getTop().intValue(), 
					section.endPoint.getLeft().intValue(), 
					section.endPoint.getTop().intValue(), 
					section.isVertical()));
		}
	}
	...
	public void moveOffsetFromStartPos(int xOffset, int yOffset) {
		...
		for (int i = 0 ; i < sections.size() ; i++) {
			...
			diagram.boundaryPanel.setWidgetPosition(
					sections.get(i), 
					sectionTopLeft.getLeft() + xOffset, 
					sectionTopLeft.getTop() + yOffset);
			...
			if (sections.get(i).startPointDecoration != null) {
				this.startPointDecoration.update( 
						sections.get(i).calculateStartPointDecorationDirection(), 
			...
			if (sections.get(i).endPointDecoration != null) {
				this.endPointDecoration.update( 
						sections.get(i).calculateEndPointDecorationDirection(), 
